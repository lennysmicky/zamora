...
const RestaurantsPage = () => {
  ...
  const filteredRestaurants = restaurants.filter(r => {
    ...
    const matchStatus = !filters.status ||
      (filters.status === 'active' && r.status === 'Ouvert') ||
      (filters.status === 'inactive' && r.status === 'Fermé');
  ...
  const stats = {
    ...
    active: restaurants.filter(r => r.status === 'Ouvert').length,
    inactive: restaurants.filter(r => r.status === 'Fermé').length
  };
  ...
  // ----- Toggle Status immédiat et persistant -----
  const handleToggleStatus = async (restaurant) => {
    ...
    setRestaurants(prev =>
      prev.map(r =>
        getId(r) === getId(restaurant) ? { ...r, status: newStatus } : r
      )
    );
    ...
    try {
      ...
      setRestaurants(prev =>
        prev.map(r =>
          getId(r) === getId(restaurant) ? { ...r, status: restaurant.status } : r
        )
      );
      alert(err.response?.data?.message || t('restaurants.errors.update', 'Erreur lors de la mise à jour'));
    }
  };
